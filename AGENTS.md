...
│   │   ├── alert-dialog-confirm.tsx    # Reusable confirmation dialog
...
│   │   ├── password-input.tsx          # Password input with eye toggle (InputGroup)
│   │   ├── search-input.tsx            # Debounced search input
│   │   ├── task-actions-dropdown.tsx   # Dropdown: Edit, Mark as, Delete
│   │   ├── task-filters.tsx            # Search + Status + Priority filter bar
│   │   ├── task-form.tsx               # Reusable create/edit task form (RHF + Zod)
│   │   ├── theme-toggle.tsx            # Theme toggle button
│   │   └── user-actions-dropdown.tsx   # Dropdown: Activate/Inactivate, Delete
│   └── ui/                     # Base UI / Ark UI primitives (DO NOT MODIFY)
...
│   ├── use-dashboard.ts        # Dashboard overview hooks (Zustand-backed)
│   ├── use-debounce.ts         # Debounce hook for search
│   ├── use-image-upload.ts     # Immediate image upload (on file select)
│   ├── use-mobile.ts           # Responsive breakpoint detection
│   ├── use-task.ts             # Task CRUD hooks (TanStack Query)
│   └── use-user.ts             # User management hooks (TanStack Query)
...
│   ├── home-page.tsx           # Landing page (motion animations)
│   ├── sign-in-page.tsx        # Auth: sign in
│   ├── sign-up-page.tsx        # Auth: sign up (immediate image upload)
│   ├── dashboard-overview.tsx  # Role-aware overview switch
│   ├── add-task-page.tsx       # User: create task (reuses TaskForm)
│   ├── my-tasks-page.tsx       # User: task list + edit dialog with TaskForm
...
│   └── admin-all-users-page.tsx # Admin: user management (toggle active, delete)
...
│   ├── dashboard.ts            # Dashboard overview API (user/admin stats)
│   ├── task.ts                 # Task CRUD API
│   └── user.ts                 # User management API (list, toggle, delete)
├── stores/                     # Zustand stores
│   ├── auth-store.ts           # Authentication state (user, isLoading)
│   ├── dashboard-store.ts      # Dashboard overview (no cache, fetch on mount)
...
│   └── user.ts                 # Role enum + TUser type
├── utils/                      # Standalone utility functions
│   ├── get-error-message.ts    # Error message extraction utility
...
| Base UI         | Headless primitives (Dialog, Dropdown, Button, ScrollArea, etc.) |
| TanStack Query  | Server state management (tasks, users)                           |
| Zustand         | Client state (auth, filter/search, dashboard overview)           |
| React Hook Form | Form state management                                            |
| Zod v4          | Schema validation                                                |
| Axios           | HTTP client with interceptors                                    |
| sonner          | Toast notifications                                              |
| next-themes     | Dark/light theme switching                                       |
| Motion          | Animation library (home page, scroll-triggered animations)       |
| React Router v8 | Client-side routing (data mode)                                  |

## Routing

All dashboard routes live under `/dashboard`. The root layout hides
Navbar/Footer for any path starting with `/dashboard`. The dashboard layout
provides a sticky header and a fixed sidebar with independent scrolling via
`ScrollArea`.
...
- `ProtectedRoute` component wraps authenticated routes with optional role
  filter
- The outer `ProtectedRoute` (no `allowedRoles`) blocks unauthenticated users
- Inner `ProtectedRoute` (with `allowedRoles`) redirects unauthorized roles to
  `/dashboard`

## State Management

- **TanStack Query**: Server state for tasks and users (with cache invalidation)
- **Zustand**:
  - `auth-store`: Authentication state with standalone async actions
  - `dashboard-store`: Dashboard overview data (no cache — fresh fetch on mount
    via `useEffect`)
  - `task-filter-store`: Table filter/search/pagination state
- **React Hook Form**: Form state per form instance (with Zod resolver)
...
- `FileUpload` component shows preview and calls `onFileSelect` with the raw
  `File`
...
Each feature has its own `services/` folder with API call functions, wrapped in
...
Uses a simple `DataTableColumn<T>` API (not TanStack Table):
...
### TaskForm (`components/shared/task-form.tsx`)

Reusable form for create/edit with React Hook Form + Zod validation:

- Props: `defaultValues`, `onSubmit`, `submitLabel`, `isSubmitting`, `onCancel`
- Used by `AddTaskPage` (create) and `MyTasksPage` (edit in dialog)

### PasswordInput (`components/shared/password-input.tsx`)

Password field with show/hide toggle using `InputGroup`:

- Uses `InputGroup` + `InputGroupInput` + `InputGroupAddon` + `InputGroupButton`
- Manages visibility state internally
- Used in both `SignInPage` and `SignUpPage`

### Action Dropdowns

- `TaskActionsDropdown`: Edit, Mark as status, Delete
- `UserActionsDropdown`: Activate/Inactivate, Delete

Both are used in table columns and mobile card views.
...
- **Toast notifications** via `sonner` (`toast.success()`, `toast.error()`,
  `toast.loading()`)
- **Base UI `render` prop** for Link-in-Button composition (not `asChild`)
- **Motion** for scroll-triggered animations (`whileInView`, `whileHover`)
